...
public class LeaderboardController {
	...
	public ResponseEntity<String> getTopScorersLeaderboard(@PathVariable String numberOfScorers) {
		try {
			...
			List<LeaderboardResponseDTO> result = leaderboardService.getTopScorersLeaderboard(Long.parseLong(numberOfScorers));
			...
			for(int i=0; i<result.size(); i++) {
				LeaderboardResponseDTO obj = result.get(i);
				responseDataString += "Name =  " + obj.getUserNameInfo() + " And Score =  " + obj.getTotalScoresValue()+ "\n";
